...
class WarningActivity : BaseSubActivity() {
    ...
    private val payload by lazy { intent?.data.toString().substringAfter("BigDefiwarn:") }
        ...
        when (item.itemId) {
            ...
            R.id.menu_mail -> true.also {
                startIntentWithCatch(Intent.createChooser(Intent(Intent.ACTION_SEND).apply {
                    type = "plain/text";
                    putExtra(Intent.EXTRA_EMAIL, arrayOf("BigDefi@BigDefi.org"))
                    putExtra(Intent.EXTRA_SUBJECT, "[BigDefi Warning]");
                    putExtra(Intent.EXTRA_TEXT, currentWarning);
                }, "Send mail"))
            }
        }
